...
try:
    ...
    print(f"Missing required library: {missing_lib}. Installing...")
    os.system(f"pip install {missing_lib}")

# Mapping of platforms to their respective API endpoints
platform_api_endpoints = {
    'onlyfans': 'https://coomer.su/api/v1/onlyfans/user/{user_id}',
    'fansly': 'https://coomer.su/api/v1/fansly/user/{user_id}',
    'candfans': 'https://coomer.su/api/v1/candfans/user/{user_id}'
}
...
def fetch_artist_data(artist_url):
    ...
    while True:
        api_url = f"https://coomer.su/api/v1/{platform}/user/{user_id}?o={offset}"
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for post in artist_data:
                if 'attachments' in post:
                    for attachment in post['attachments']:
                        file_name = attachment['name']
                        file_url = f'https://coomer.su{attachment["path"]}'
